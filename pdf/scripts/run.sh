...
usage() {
  cat <<'EOF'
用法:
  bash pdf/scripts/run.sh replace-page <src_pdf> <repl_pdf> <page> <out_pdf>
  bash pdf/scripts/run.sh overlay-watermark <src_pdf> <out_pdf> [args...]
  bash pdf/scripts/run.sh watermark <src_pdf> <out_pdf> [args...]
  bash pdf/scripts/run.sh seam-seal <input_pdf> <seal_png> <output_pdf> [args...]
  bash pdf/scripts/run.sh page-ops <subcommand> [args...]
  bash pdf/scripts/run.sh images-to-pdf <output_pdf> <img1> <img2> ...
  bash pdf/scripts/run.sh pdf-to-images <input_pdf> <output_dir> [args...]
  bash pdf/scripts/run.sh batch <subcommand> [args...]
  bash pdf/scripts/run.sh check
  bash pdf/scripts/run.sh bootstrap
EOF
}
...
case "$command_name" in
  ...
  batch)
    ...
    ;;
  check)
    exec "$PYTHON_BIN" "$SCRIPT_DIR/check_runtime.py" "$@"
    ;;
  bootstrap)
