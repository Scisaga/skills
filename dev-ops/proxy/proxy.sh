...
Usage: $(basename "$0") [--proxy-port 8888] [--allow-cidr 10.6.0.0/24] [--with-warp]

Installs tinyproxy and enables WireGuard forwarding.
...
sudo apt-get update
sudo apt-get install -y tinyproxy wireguard resolvconf curl ufw
...
if [ "${WITH_WARP}" = "true" ]; then
  ...
  sudo apt-get install -y gpg lsb-release
