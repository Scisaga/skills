...
3. 确认运行前置条件。
   ...
   - 首次使用前优先执行 `speech/scripts/bootstrap.sh` 完成依赖安装和环境检查。
...
5. 运行入口。
   - 统一入口：`bash speech/scripts/run.sh`
   - 合成入口：`bash speech/scripts/run.sh synthesize`
   - 转写入口：`bash speech/scripts/run.sh transcribe`
   - 环境检查入口：`bash speech/scripts/run.sh doctor`
   - 音频输出用 `--output-mp3`。
...
bash speech/scripts/run.sh synthesize --text "你好，欢迎使用语音合成。" --output-mp3 out/demo.mp3
bash speech/scripts/run.sh synthesize --input-file script.txt --voice zh-CN-XiaoxiaoNeural --style cheerful
cat notes.txt | bash speech/scripts/run.sh synthesize --stdin --output-mp3 out/notes.mp3
bash speech/scripts/run.sh transcribe --input-file demo.mp3 --output-text out/demo.txt
bash speech/scripts/run.sh transcribe --input-file meeting.wav --language zh --prompt "产品名 Servforce.AI"
```
