...
```bash
bash speech/scripts/bootstrap.sh --check-only
```
...
```bash
bash speech/scripts/run.sh synthesize --text "欢迎来到演示。" --output-mp3 out/demo.mp3
```

Read from file:

```bash
bash speech/scripts/run.sh synthesize --input-file script.txt --output-mp3 out/demo.mp3
```

Read from stdin:

```bash
cat script.txt | bash speech/scripts/run.sh synthesize --stdin --output-mp3 out/demo.mp3
```
...
```bash
bash speech/scripts/run.sh transcribe --input-file demo.mp3 --output-text out/demo.txt
```

With known language and prompt:

```bash
bash speech/scripts/run.sh transcribe \
...
```bash
bash speech/scripts/run.sh transcribe --input-file meeting.wav --output-json out/meeting.json
```
...
- `bash speech/scripts/run.sh bootstrap`: 安装依赖并检查环境
- `bash speech/scripts/run.sh doctor`: 仅检查环境
- `bash speech/scripts/run.sh synthesize ...`: 运行 TTS
- `bash speech/scripts/run.sh transcribe ...`: 运行 ASR
...
- Missing dependency: run `bash speech/scripts/bootstrap.sh`.
