...
未来路线、技术愿景和商业蓝图页按 `vision-and-roadmap-narrative.md` 做人工语义验收，并将结论写入 content、visual 或 narration 审批摘要及内部 QA。验收同时确认：

- 愿景先行：主体先呈现方向、目标状态和价值，不让防御性限定抢占标题或核心视觉；
- 主张清晰：标题、关键标签、正文和旁白共同形成可复述的主张及规模化逻辑；
- 边界克制：必要限定集中放置且不重复，法律、安全、财务或重大事实风险仍保持清楚可见；
- 事实不失真：当前、进行中和目标态明确区分，没有把规划能力包装成已经实现的事实。

若 `identity` 逐页稿不满足这项标准，只报告具体问题和建议，不得静默改写。该检查依赖语义判断，不伪装成 static、audio 或 standard 脚本已自动证明的结果。
...
- 表现 profile 由语速与非末段停顿/断句组合建立；音高稳定为 `+0st` 不会误判，机械交替 `-0.1st/+0.1st` 也不能单独通过；
- 最终音高审计逐段计算 `global_pitch + local_pitch`，报告最小值、最大值和越界页码/段落；默认任一结果超出 `±0.1st` 即失败，不自动 clamp；
...
- 同一 voice、style 和全局声音配置贯穿全部章节，没有章节间说话人身份漂移；
