...
description: 在 Android 设备上安装、迁移、配置和验证可独立运行的 Termux SSH、Termux:Boot、Termux:API、共享存储、蓝牙 companion、Bash 与可选本机 Codex CLI。适用于通过 ADB 完成一次性部署后，要求设备通过局域网 SSH 自启动、息屏可达，或作为无需笔记本的网络探针和 IoT Agent 使用的场景。
---
...
5. 把本 skill 的 `scripts/bootstrap.sh` 与 `scripts/update-agents-md.sh` 放到 Termux 的同一目录；需要本机 Codex 时一并复制 `scripts/configure-codex.sh`。使用公钥文件或标准输入执行 bootstrap，它会在登录根目录生成/更新 `~/AGENTS.md`，保留其中非受管内容。不要把私钥复制到设备，也不要把真实公钥、IP 或序列号写回 skill。
6. 需要相机、Wi-Fi、共享存储或 Ubuntu 风格 Bash 时，读 [references/remote-management.md](references/remote-management.md)。需要蓝牙状态、已配对列表或 BLE 扫描时，额外读 [references/bluetooth-companion.md](references/bluetooth-companion.md)。需要让平板本机运行 Codex 时，读 [references/codex-on-termux.md](references/codex-on-termux.md)。相机测试结束后删除测试照片并验证不存在残留。
...
# 可选：创建其他 Android 应用可见、但不改变 SSH 登录目录的工作区别名
bash bootstrap.sh --authorized-key-file ~/host_key.pub \
...
# 可选：在原生 Termux 安装官方 Codex；无沙箱必须由用户显式选择
bash configure-codex.sh --proxy http://PROXY_HOST:PORT
bash configure-codex.sh --skip-install \
  --proxy http://PROXY_HOST:PORT --no-sandbox

# 仅在用户明确要求 Full access 时使用
bash configure-codex.sh --skip-install \
  --proxy http://PROXY_HOST:PORT --full-access
...
- 原生 Termux 上的 Codex 安装、代理、CA、登录、Android 沙箱限制和设备 Agent 模式：读 `references/codex-on-termux.md`。
